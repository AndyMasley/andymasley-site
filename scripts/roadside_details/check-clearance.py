...
root=Path(os.environ.get('WEBSTER_ROADSIDE_WORK','/private/tmp/webster-final-details/streetscape'));repo=Path(__file__).resolve().parents[2]
...
report={'result':'pass'if not hits else'fail','scope':'Actual generated roadside fixture and utility triangles at all three LODs against oriented 4.46m-long/2.28m-wide authored car boxes, including slope/pitch, actual RoadGraph lane offsets and every offered nearby connector. No centerline-only clearance assumption. Vehicle vertical range is contact-.015m to2.1m; horizontal margin includes half the .25m sampling step plus turn allowance. This is a bounded static clearance audit, not a general physical collision engine.','sourceNetworkSHA256':source['sourceNetworkSHA256'],'engineSHA256':source['engineSourceSHA256'],'gradeSHA256':source['gradeSourceSHA256'],'recipeSHA256':hashlib.sha256(Path(repo/'data/derived/town/roadside-audit.json').read_bytes()).hexdigest(), 'legacyEnvironmentRecipeSHA256':hashlib.sha256(Path(repo/'data/derived/town/evidence-environment.json').read_bytes()).hexdigest(), 'roadsideModuleSHA256':hashlib.sha256(Path(repo/'src/lib/town/roadside-details.ts').read_bytes()).hexdigest(),'routeCount':source['routeCount'],'poseCount':source['poseCount'],'triangleCountsByLOD':by_lod,'candidateTrianglePoseChecks':checks,'intersectionCount':len(hits),'intersections':hits[:100], 'byFeature':dict(collections.Counter(','.join(h['featureIds'])for h in hits)), 'firstByFeature':{','.join(h['featureIds']):h for h in reversed(hits)},'allHits':hits}
(root/'roadside-vehicle-clearance.json').write_text(json.dumps(report,indent=2));print(json.dumps({k:v for k,v in report.items()if k!='intersections'},indent=2))
