...
root=Path(os.environ.get('WEBSTER_PROPERTY_TERRAIN','/private/tmp/webster-finished-game/art/property-terrain'))
...
rows=[];issues=[]
import json
catalog=json.loads((site/'data/derived/town/property-grounds.json').read_text())
...
assert sum(r['samples']for r in rows)>1000, 'Missing actual property mesh samples'
