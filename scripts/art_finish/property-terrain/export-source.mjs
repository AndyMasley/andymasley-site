...
const tiles=new Set(Object.keys(data('property-grounds').tiles));
for(const tile of manifest.tiles.filter(t=>tiles.has(t.id)))for(const lod of tile.lods){
 ...
 for(const step of tileAssemblySteps(group,tile,lod.level,details(tile.id,lod.level))){if(step.name==='propertyGrounds'||step.name==='propertyTerrain')break;step.apply();}
 group.updateMatrixWorld(true);const inv=group.matrixWorld.clone().invert(), terrain=[],protectedFaces=[];
 group.traverse(o=>{if(!(o instanceof THREE.Mesh))return;const g=o.geometry,p=g.getAttribute('position'),ix=g.index,m=inv.clone().multiply(o.matrixWorld),v=new THREE.Vector3();
  ...
  else if(/^(roads|water)(?:\b|_)/i.test(o.name)||/road|water|sidewalk|parking|apron/i.test(o.name))protectedFaces.push(...faces.filter(t=>Math.abs((t[1][0]-t[0][0])*(t[2][1]-t[0][1])-(t[1][1]-t[0][1])*(t[2][0]-t[0][0]))>1e-7));
 });
 const record={tileId:tile.id,level:lod.level,origin:tile.origin,sourceSha256:lod.sha256,terrain,protectedFaces,assemblySha256:hash(fs.readFileSync(path.join(site,'src/lib/town/tile-assembly.ts')))};
