...
ROOT=Path(__file__).resolve().parents[3];OUT=Path(os.environ.get('WEBSTER_MILL_YARD','/private/tmp/webster-finished-game/art/mill-yard'))
catalog=json.loads((ROOT/'data/derived/town/mill-yard-grounds.json').read_text());domains=json.loads((OUT/'geometry-domain.json').read_text());expected=shape(domains['final']);protected=shape(domains['protected']);rows=[]
for level in [0,1,2]:
 polys=[]
...
report={'status':'FAIL'if failures else'PASS','policy':'Every actual emitted mill apron triangle at each LOD, both registered tile owners; 50 micrometre projected Float32 boundary allowance. Source protected domain includes retained roads, courts, building walls, water and all nearby padded guided-car envelopes.','rows':rows,'failures':failures};(OUT/'domain-audit.json').write_text(json.dumps(report,indent=2));print(report);assert not failures
