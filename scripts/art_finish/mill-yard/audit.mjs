...
const out = path.resolve(process.env.TOWN_ASSEMBLY_OUT || '/private/tmp/webster-finished-game/art/mill-yard');
...
fs.writeFileSync(entry, `export { tileAssemblySteps } from ${JSON.stringify(path.join(site, 'src/lib/town/tile-assembly.ts'))}; export { applyMillYardGrounds } from ${JSON.stringify(path.join(site, 'src/lib/town/mill-yard-grounds.ts'))};`);
...
const { tileAssemblySteps, applyMillYardGrounds } = await import(pathToFileURL(bundle).href);
...
const catalog=data('mill-yard-grounds'),report={sourceManifestSha256:release.manifestSha256,catalogSha256:hash(fs.readFileSync(path.join(site,'data/derived/town/mill-yard-grounds.json'))),rows:[],failures:[]};
...
for(const tile of manifest.tiles.filter(t=>catalog.tiles[t.id]))for(const lod of tile.lods){
 ...
 for(const step of tileAssemblySteps(group,tile,lod.level,details(tile.id,lod.level)))if(step.name!=='millYardGrounds')step.apply();
 ...
 const start=performance.now(),result=applyMillYardGrounds(group,tile.id,tile.origin,lod.level,lod.sha256),ms=performance.now()-start,failures=[];
 ...
 if(applyMillYardGrounds(group,tile.id,tile.origin,lod.level,lod.sha256)!==result)failures.push('Repeated assembly');
 ...
 group.traverse(o=>{if(!(o instanceof THREE.Mesh))return;const isTerrain=/^terrain(?:\b|_)/i.test(o.name),isNew=o.parent?.name==='Mill Street industrial apron';if(!isTerrain&&!isNew)return;const p=o.geometry.getAttribute('position'),n=o.geometry.getAttribute('normal'),ix=o.geometry.index,m=inverse.clone().multiply(o.matrixWorld),faces=[];
