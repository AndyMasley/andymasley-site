...
fs.writeFileSync(entry, `export { tileAssemblySteps } from ${JSON.stringify(path.join(site, 'src/lib/town/tile-assembly.ts'))}; export { applyBathhouseGrounds } from ${JSON.stringify(path.join(site, 'src/lib/town/bathhouse-grounds.ts'))};`);
...
const { tileAssemblySteps, applyBathhouseGrounds } = await import(pathToFileURL(bundle).href);
...
const catalog=data('bathhouse-grounds'),report={sourceManifestSha256:release.manifestSha256,catalogSha256:hash(fs.readFileSync(path.join(site,'data/derived/town/bathhouse-grounds.json'))),rows:[],failures:[]};
...
for(const tile of manifest.tiles.filter(t=>catalog.tiles[t.id]))for(const lod of tile.lods){
 ...
 const start=performance.now(),result=applyBathhouseGrounds(group,tile.id,tile.origin,lod.level,lod.sha256),ms=performance.now()-start,failures=[];
 ...
 if(applyBathhouseGrounds(group,tile.id,tile.origin,lod.level,lod.sha256)!==result)failures.push('Repeated assembly');
 ...
 group.traverse(o=>{if(!(o instanceof THREE.Mesh))return;const isTerrain=/^terrain(?:\b|_)/i.test(o.name),isNew=o.parent?.name==='Memorial Beach bathhouse forecourt';if(!isTerrain&&!isNew)return;const p=o.geometry.getAttribute('position'),n=o.geometry.getAttribute('normal'),ix=o.geometry.index,m=inverse.clone().multiply(o.matrixWorld),faces=[];
