...
const imports=['road-finish','road-material-finish','terrain-finish','environment-ground'];
...
for(const lod of tile.lods){
 ...
 scene.traverse(o=>{
  ...
  const category=o.name==='roads'||o.parent?.name==='roads'?'roads':/^terrain(?:\b|_)/.test(o.name)?'terrain':o.name==='water'?'water':o.name.startsWith('streetscape')||o.parent?.name==='streetscape'?'streetscape':null;if(!category)return;
 ...
 fs.writeFileSync(`${out}/input-${lod.level}.json.gz`,gzipSync(JSON.stringify({tileId:tile.id,origin:tile.origin,sourceManifestSha256:release.manifestSha256,sourceSha256:lod.sha256,level:lod.level,meshes})));
...
execFileSync(process.execPath,[site+'/scripts/road_curves/export-guided-poses.mjs'],{stdio:'inherit',env:{...process.env,ROAD_CURVE_WORK:out}});
execFileSync(process.env.PYTHON??'python3',[site+'/scripts/road_curves/prepare.py'],{stdio:'inherit',env:{...process.env,ROAD_CURVE_WORK:out}});
