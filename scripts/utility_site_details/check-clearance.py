...
import os
root=Path(os.environ.get('WEBSTER_UTILITY_QA','/private/tmp/webster-final-details/utilities'))
...
manifest=json.loads((root/'utility-native-audit.json').read_text())
assert manifest['status']=='PASS'
assert manifest['runtimeSha256']==hashlib.sha256((repo/'src/lib/town/utility-site-details.ts').read_bytes()).hexdigest()
assert manifest['dataSha256']==hashlib.sha256((repo/'data/derived/town/utility-site-details.json').read_bytes()).hexdigest()
for audit in manifest['results']:
 path=Path(audit['geometryExport']['path']);blob=path.read_bytes();assert hashlib.sha256(blob).hexdigest()==audit['geometryExport']['sha256'];row=json.loads(gzip.decompress(blob));row['geometry']=[dict(x,position=x['positions'])for x in row['meshes']]
...
report={'result':'pass'if not hits else'fail','scope':'Actual generated utility basin, rail and walkway triangles at all three LODs against oriented 4.46m-long/2.28m-wide authored car boxes, including slope/pitch, actual RoadGraph lane offsets and every offered nearby connector. No centerline-only clearance assumption. Vehicle vertical range is contact-.015m to2.1m; horizontal margin includes half the .25m sampling step plus turn allowance. This is a bounded static clearance audit, not a general physical collision engine.','sourceNetworkSHA256':source['sourceNetworkSHA256'],'engineSHA256':source['engineSourceSHA256'],'gradeSHA256':source['gradeSourceSHA256'],'recipeSHA256':hashlib.sha256((repo/'data/derived/town/utility-site-details.json').read_bytes()).hexdigest(), 'nativeAuditSHA256':hashlib.sha256((root/'utility-native-audit.json').read_bytes()).hexdigest(), 'utilityModuleSHA256':hashlib.sha256((repo/'src/lib/town/utility-site-details.ts').read_bytes()).hexdigest(),'routeCount':source['routeCount'],'poseCount':source['poseCount'],'triangleCountsByLOD':by_lod,'candidateTrianglePoseChecks':checks,'intersectionCount':len(hits),'intersections':hits[:100], 'byFeature':dict(collections.Counter(','.join(h['featureIds'])for h in hits)), 'firstByFeature':{','.join(h['featureIds']):h for h in reversed(hits)},'allHits':hits}
(root/'utility-vehicle-clearance.json').write_text(json.dumps(report,indent=2));print(json.dumps({k:v for k,v in report.items()if k not in ['intersections','allHits','firstByFeature']},indent=2))
