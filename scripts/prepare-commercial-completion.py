...
out={'version':1,'sourceManifestSha256':sha(base/'manifest.json'),'sourceLedgerSha256':sha(RESEARCH/'implementation/landmark-evidence.json'),'coordinateSystem':'east,north,source-local height metres; Three X,height,-north; origin applied once','rules':['Only explicit source GLB SHA/LOD/origin match may render.','Source bodies retained except the documented Gilles/Tiffany height correction.','Shared footprints are allocated by actual address parcels; no blanket shared-footprint recolors.','Historical/current observations and authored detail assumptions remain separate; no business logo textures.','September 7 finish: display-pane spacing, high transoms, stall risers, shallow fascia and separated door openings are authored architectural inference, not a claim of photographed current shop joinery. Gilles/Tiffany secondary windows follow each retained wing floor/eave and omit attached party walls; their exact rhythm is inferred.'],'rows':rows}
