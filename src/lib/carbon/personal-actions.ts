...
export function computePersonalActions(baseline: BaselineInputs, footprint: FootprintModel): PersonalAction[] {
  ...
  const actions: PersonalAction[] = [
    ...
    },
    {
      ...
      note: `Eliminates all ${baseline.flightsPerYear} flight${baseline.flightsPerYear !== 1 ? 's' : ''} per year`,
      inlineParam: { before: 'Eliminate all ', defaultVal: baseline.flightsPerYear, after: ` flight${baseline.flightsPerYear !== 1 ? 's' : ''}` },
