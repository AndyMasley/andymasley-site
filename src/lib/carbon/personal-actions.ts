...
export function computePersonalActions(baseline: BaselineInputs, footprint: FootprintModel): PersonalAction[] {
  ...
  const actions: PersonalAction[] = [
...

    {
      ...
      savingsKg: Math.round(100 * 0.05 * 365 / 1000), // 0.05g marginal × 100/day = 2 kg/yr
      category: 'Digital',
      applicable: true,
      note: '~0.05 g CO₂ per email (server + transmission only). Negligible.',
      inlineParam: { before: 'Send ', defaultVal: 100, after: ' fewer emails/day', max: 100000 },
    },
    {
    ...
    // ── Flights ──
    {
      ...
      savingsKg: baseline.transatlanticFlightsPerYear > 0 ? Math.round(6900 * 0.255) : 0,
      category: 'Flights',
    ...
    },
    {
      ...
      savingsKg: baseline.transpacificFlightsPerYear > 0 ? Math.round(12400 * 0.255) : 0,
      category: 'Flights',
    ...
    },
    {
      ...
      savingsKg: baseline.domesticFlightsPerYear > 0 ? Math.round(2200 * 0.255) : 0,
      category: 'Flights',
