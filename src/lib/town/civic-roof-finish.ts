...
export type CivicRoofReport={status:'applied'|'source-mismatch';triangles:number;removedTriangles?:number;boundaryFragments?:number;roofHeight?:number;enclosureTriangles?:number;enclosureArea?:number};
...
export function applyCivicRoofFinish(group:THREE.Group,tileId:string,level:number,sha256:string):CivicRoofReport|undefined{
 ...
 const row=data.lods.find(r=>r.level===level),reject=():CivicRoofReport=>({status:'source-mismatch',triangles:0});if(!row||row.sha256!==sha256||data.sourceManifestSha256!==release.manifestSha256)return reject();
 if(group.userData.civicRoofFinish)return group.userData.civicRoofFinish as CivicRoofReport;
 ...
 // The brick material belongs to the same source-qualified native body. Its
 // transform must have succeeded before this cap can retire the height step.
 const wallMatches:THREE.Mesh[]=[];group.traverse(o=>{if(o instanceof THREE.Mesh&&o.name===data.enclosure.wallMeshName&&o.parent?.name===data.enclosure.wallParentName)wallMatches.push(o);});
 const wall=wallMatches[0],wallMaterials=wall&&(Array.isArray(wall.material)?wall.material:[wall.material]),brick=wallMaterials?.find(m=>m.name===data.enclosure.brickMaterial&&m.userData.structId===data.structId);
 if(wallMatches.length!==1||!brick||wallMaterials?.[0]?.name!==data.enclosure.wallSourceMaterial||(wall.geometry.index?.count??wall.geometry.getAttribute('position')?.count)!==row.enclosure.wallTotalTriangles*3)return reject();
 if(Object.values(old.attributes).some(a=>!(a instanceof THREE.BufferAttribute)||!(a.array instanceof Float32Array)||a.normalized))return reject();
 ...
 const enclosureStart=output.position.length/3;let enclosureTriangles=0,enclosureArea=0;
 for(const face of row.enclosure.triangles){
  const edge=data.enclosure.edges.find(e=>e.id===face.edge)!;
  const dx=edge.b[0]-edge.a[0],dn=edge.b[1]-edge.a[1],length=Math.hypot(dx,dn);
  const outward=new THREE.Vector3(-dn/length,0,-dx/length).applyMatrix3(new THREE.Matrix3().getNormalMatrix(inverse)).normalize();
  const points=face.points.map(p=>new THREE.Vector3(p[0]-data.origin[0],p[2]-data.origin[1],-p[1]-data.origin[2]).applyMatrix4(inverse));
  // Test the actual float32 GPU coordinates, so tiny source-boundary slivers
  // cannot become degenerate triangles after conversion to local geometry.
  points.forEach(p=>p.set(Math.fround(p.x),Math.fround(p.y),Math.fround(p.z)));
  const cross=points[1].clone().sub(points[0]).cross(points[2].clone().sub(points[0]));if(cross.lengthSq()<1e-16)continue;
  if(cross.dot(outward)<0)points.reverse();
  enclosureArea+=cross.length()/2;enclosureTriangles++;
  for(const point of points)for(const[name,a]of Object.entries(old.attributes)){
   const values=output[name];if(name==='position')values.push(...point.toArray());else if(name==='normal')values.push(...outward.toArray());else if(name==='uv')values.push(point.x*.5,point.y*.5);else for(let k=0;k<a.itemSize;k++)values.push(0);
  }
 }
 if(!enclosureTriangles||Math.abs(enclosureArea-row.enclosure.area)>.003)return reject();
 ...
 geometry.addGroup(0,retainedCount,0);geometry.addGroup(retainedCount,triangles.length*3,1);geometry.addGroup(enclosureStart,enclosureTriangles*3,2);geometry.computeBoundingBox();geometry.computeBoundingSphere();geometry.userData={...old.userData,civicSchoolRoofFinish:true};
 ...
 mesh.geometry=geometry;mesh.material=[mesh.material,finish,brick];let referenced=false;group.traverse(o=>{if(o instanceof THREE.Mesh&&o.geometry===old)referenced=true;});if(!referenced)old.dispose();
 const report:CivicRoofReport={status:'applied',triangles:triangles.length,removedTriangles:row.triangles,boundaryFragments:row.fragments.length,roofHeight:data.roofHeight,enclosureTriangles,enclosureArea};group.userData.civicRoofFinish=report;return report;
}
