/**
 * Phase 5 — Export results as PNG, CSV, and JSON
 */

import type { FootprintModel, BucketResult } from '@/lib/carbon/types';
...
function toJSON(footprint: FootprintModel): string {
  const exportData = {
    ...
    calculatorUrl: 'https://andymasley.com/visuals/carbon-footprint-v2',
  };
...
function downloadPNG(footprint: FootprintModel) {
  const W = 600;
  const H = 400;
  ...
  const pct = Math.round(footprint.totalKgCO2ePerYear / 16000 * 100);
  ctx.fillText(`${pct}% of US average (16,000 kg)`, 30, 178);
  ...
  ctx.fillText('andymasley.com/visuals/carbon-footprint-v2', 30, H - 20);
...
export function ExportButton({ footprint }: ExportButtonProps) {
  return (
    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
      <ExportBtn label="PNG" onClick={() => downloadPNG(footprint)} />
